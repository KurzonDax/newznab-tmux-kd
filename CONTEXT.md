...
**Clip**:
The full-resolution, stream-copied (no transcode) form of the Generated Sample Video, covering the whole downloaded head window. Produced only where Clip storage is enabled for the release's root category (a Movies/TV/XXX toggle, default XXX only) and the source codecs are browser-playable without transcoding; otherwise no video artifact is stored at all (Clip-or-nothing). An encode shorter than the duration floor (`clip_minimum_seconds`, default 5, 0 disables) is discarded the same way — a starved extraction stores no video artifact rather than a seconds-long tease; an unreadable duration is not "below the floor". Lives and dies with its release, and is never generated while the covers volume is under the Free-disk guard.
