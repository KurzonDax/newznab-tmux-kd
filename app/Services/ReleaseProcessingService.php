...
final class ReleaseProcessingService
{
    ...
    public function createNZBs(int|string|null $groupID): int
    {
        ...
        if ($deferredCount > 0) {
            $this->outputStat('NZB creation deferred', $deferredCount);
        }
    ...
    private function handleFailedNzbCreation(
        ...
        string $claimToken,
    ): NzbCreationFailureDisposition {
        if ($result->isDeterministicFailure()) {
            $deleted = $this->deleteFailedNzbCreationRelease(
                $release,
                $result,
                $claimToken,
            ...
            return $deleted ? NzbCreationFailureDisposition::Deleted : NzbCreationFailureDisposition::ClaimLost;
        }

        $failure = $this->persistNzbCreationFailure($release, $result, $claimToken);
        if ($failure === null) {
            return NzbCreationFailureDisposition::ClaimLost;
        }
        ...
        return NzbCreationFailureDisposition::Retry;
    }
    ...
    private function persistNzbCreationFailure(
        ...
        $failure = DB::transaction(function () use ($release, $result, $claimToken): ?ReleaseNzbCreationFailure {
            $ownedRelease = NzbCreationCandidateQuery::ownedPendingBuilder((int) $release->id, $claimToken);
            if (DB::getDriverName() !== 'sqlite') {
                $ownedRelease->lockForUpdate();
            }

            if ($ownedRelease->first(['id']) === null) {
                return null;
            }
            ...
            if (! $failure instanceof ReleaseNzbCreationFailure) {
                return ReleaseNzbCreationFailure::query()->create([
            ...
            $failure->attempts++;
            $failure->last_error = mb_substr($result->reason, 0, 1000);
            $failure->save();
    ...
    private function deleteFailedNzbCreationRelease(
        ...
        return DB::transaction(function () use ($release, $result, $claimToken, $attempt): bool {
            $ownedRelease = NzbCreationCandidateQuery::ownedPendingBuilder((int) $release->id, $claimToken);
            if (DB::getDriverName() !== 'sqlite') {
                $ownedRelease->lockForUpdate();
            }
            ...
            DB::afterCommit(static fn () => Log::channel('nzb_creation')->warning('Deleting release after NZB creation failure', [
                'release_id' => $release->id,
                'guid' => $release->guid,
                'failure_type' => $result->failureType,
                'reason' => $result->reason,
    ...
    private function deleteReleasesUnderMinSize(int|string $groupId, array &$stats): void
    {
        ...
        foreach ($releases as $release) {
            if ($this->deleteSingleRelease($release, 'deleteReleasesUnderMinSize')) {
                $stats['minSize']++;
            }
        }
    }
    ...
    private function deleteReleasesOverMaxSize(int|string $groupId, array &$stats): void
    {
        ...
        foreach ($releases as $release) {
            if ($this->deleteSingleRelease($release, 'deleteReleasesOverMaxSize')) {
                $stats['maxSize']++;
            }
        }
    }
    ...
    private function deleteReleasesUnderMinFiles(int|string $groupId, array &$stats): void
    {
        ...
        foreach ($releases as $release) {
            if ($this->deleteSingleRelease($release, 'deleteReleasesUnderMinFiles')) {
                $stats['minFiles']++;
            }
        }
    }
    ...
    private function deletePasswordedReleases(ReleaseDeleteStats $stats): ReleaseDeleteStats
    {
        ...
        $this->releaseSweepQuery()
            ...
            ->chunkById(self::BATCH_SIZE, function ($releases) use (&$stats): bool {
                foreach ($releases as $release) {
                    if ($this->deleteSingleRelease($release, 'deletePasswordedReleases')) {
    ...
    private function deleteCrossPostedReleases(ReleaseDeleteStats $stats): ReleaseDeleteStats
    {
        ...
        foreach ($releases as $release) {
            if ($this->deleteSingleRelease($release, 'deleteCrossPostedReleases')) {
    ...
    private function deleteDisabledCategoryReleases(ReleaseDeleteStats $stats): ReleaseDeleteStats
    {
        ...
        $this->releaseSweepQuery()
            ...
            ->chunkById(self::BATCH_SIZE, function ($releases) use (&$stats): bool {
                foreach ($releases as $release) {
                    if ($this->deleteSingleRelease($release, 'deleteDisabledCategoryReleases')) {
    ...
    private function deleteCategoryMinSizeReleases(ReleaseDeleteStats $stats): ReleaseDeleteStats
    {
        ...
        foreach ($categories as $category) {
            $this->releaseSweepQuery()
                ...
                ->chunkById(self::BATCH_SIZE, function ($releases) use (&$stats): bool {
                    foreach ($releases as $release) {
                        if ($this->deleteSingleRelease($release, 'deleteCategoryMinSizeReleases')) {
    ...
    private function deleteDisabledGenreReleases(ReleaseDeleteStats $stats): ReleaseDeleteStats
    {
        ...
        foreach ($genreList as $genre) {
            ...
            $this->releaseSweepQuery()
                ...
                ->chunkById(self::BATCH_SIZE, function ($releases) use (&$stats): bool {
                    foreach ($releases as $release) {
                        if ($this->deleteSingleRelease($release, 'deleteDisabledGenreReleases')) {
    ...
    private function deleteMiscReleases(ReleaseDeleteStats $stats): ReleaseDeleteStats
    {
        if ($this->settings->miscOtherRetentionHours > 0) {
            ...
            $this->releaseSweepQuery()
                ...
                ->chunkById(self::BATCH_SIZE, function ($releases) use (&$stats): bool {
                    foreach ($releases as $release) {
                        if ($this->deleteSingleRelease($release, 'deleteMiscReleases')) {
    ...
    private function deleteSingleRelease(Release $release, string $reason): bool
    {
        return $this->releaseManagement->deleteSingleIfUnclaimed(
            ...
            $this->releaseImage,
            reason: $reason,
            evidence: static function (Release $current) use ($release, $reason): array {
                $unchanged = $current->getRawOriginal() === $release->getRawOriginal();
                if ($reason === 'deletePasswordedReleases') {
