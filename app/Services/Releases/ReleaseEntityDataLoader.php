...
{
    /** @param Collection<int|string, object> $releases
    ...
    public function load(Collection $releases): array
    {
        ...
        foreach ($sources as $categoryRoot => [$root, $table, $foreignKey, $key, $yearField, $artType]) {
            ...
            $records = DB::table($table)->whereIn($key, $matched->pluck($foreignKey))->get([$key, 'title', $yearField])->keyBy($key);
            $episodeIds = $root === 'tv' ? $matched->pluck('tv_episodes_id')->filter()->unique() : collect();
            $episodes = $episodeIds->isEmpty() ? collect() : DB::table('tv_episodes')->whereIn('id', $episodeIds)->get(['id', 'series', 'episode', 'videos_id'])->keyBy('id');
        ...
        if ($anime->isNotEmpty()) {
            $ranked = DB::table('anidb_info as info')->join('anidb_titles as titles', 'titles.anidbid', '=', 'info.anidbid')
                ->whereIn('info.anidbid', $anime->pluck('anidbid'))
                ->select(['info.anidbid', 'info.startdate', 'titles.title'])
