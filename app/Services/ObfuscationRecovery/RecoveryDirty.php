...
final class RecoveryDirty
{
    ...
    public static function mark(Connection $connection, array $rows): void
    {
        ...
        foreach ($rows as $row) {
            ...
            $bucket = intdiv((int) $row['embedded_timestamp_ms'], 60000);
            $key = $scope.':'.$bucket;
            $keys[$key] ??= ['scope_digest' => $scope, 'source_epoch' => $row['source_epoch'], 'groups_id' => $row['groups_id'],
                'capture_generation' => $row['capture_generation'], 'profile' => $row['profile'], 'partition_value' => $partition,
                'bucket' => $bucket, 'first_ms' => $row['embedded_timestamp_ms'], 'last_ms' => $row['embedded_timestamp_ms'],
                'version' => 1, 'membership_changed_at' => now(), 'next_action_at' => now()];
            $keys[$key]['first_ms'] = min($keys[$key]['first_ms'], $row['embedded_timestamp_ms']);
            $keys[$key]['last_ms'] = max($keys[$key]['last_ms'], $row['embedded_timestamp_ms']);
        }
        if ($keys === []) {
            return;
        }
        ...
        foreach (array_chunk(array_values($keys), 250) as $chunk) {
            $connection->table('obfuscation_recovery_dirty')->upsert($chunk, ['scope_digest', 'bucket'], [
