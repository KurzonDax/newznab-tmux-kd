...
## Audio selection and probing

`AudioCandidateQuery` selects music-routed candidates with no minimum size;
applying `minsizetopostprocess` strands legitimate small audio releases. The global
maximum still applies. `AudioReleaseProcessor` and its fetcher probe article 1
before pulling the rest of the head; selection itself performs no NNTP fetches.
Audio tunables belong in the Post-Processing settings provider and seeder.
