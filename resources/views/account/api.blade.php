...
<section class="card account-card" x-data="accountRss" data-rss-base="{{ url('/rss') }}" data-api-token="{{ $user->api_token }}">
    ...
    <div class="account-form">
        <div><x-label for="feed-type">Feed</x-label><x-select id="feed-type" x-model="feed" @change="build"><option value="full-feed">All releases</option><option value="mymovies">My Movies</option><option value="myshows">My Shows</option><option value="cart">Basket</option><option value="category">Category</option></x-select></div>
        <div x-show="feed === 'category'"><x-label for="feed-category">Category</x-label><x-select id="feed-category" x-model="category" @change="build">@foreach($categoriesWithSubs as $root)<optgroup label="{{ $root->title }}">@foreach($root->categories as $category)<option value="{{ $category->id }}">{{ $category->title }}</option>@endforeach</optgroup>@endforeach</x-select></div>
        <div><x-label for="feed-count">Results</x-label><x-select id="feed-count" x-model="count" @change="build">@foreach([25, 50, 100] as $count)<option value="{{ $count }}">{{ $count }}</option>@endforeach</x-select></div>
