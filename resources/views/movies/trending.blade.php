...
<div class="surface-panel rounded-xl shadow-sm">
    ...
    <x-page-header title="Trending Movies" description="Most downloaded movies in the last 48 hours - Updated every hour" icon="fas fa-fire">
        <x-slot:actions>
            @auth
                <x-button-link href="{{ url('/rss/trending-movies?dl=1&api_token=' . auth()->user()->api_token) }}" variant="secondary" size="sm" icon="fas fa-rss">RSS Feed</x-button-link>
            @endauth
            <x-button-link href="{{ route('Movies') }}" variant="secondary" size="sm" icon="fas fa-compass">Browse All Movies</x-button-link>
        </x-slot:actions>
    </x-page-header>
