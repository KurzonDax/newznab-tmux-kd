    ...
    <section class="card account-card">
        ...
        @else
            ...
            <form method="POST" action="{{ route('invitations.store') }}" class="account-form mt-4">@csrf
                ...
                <div><x-label for="expiry_days">Expires after</x-label><x-select width="compact" name="expiry_days" id="expiry_days">@foreach([1, 3, 7, 14, 30] as $days)<option value="{{ $days }}" @selected((int) old('expiry_days', 7) === $days)>{{ $days }} {{ $days === 1 ? 'day' : 'days' }}</option>@endforeach</x-select></div>
                @if(!empty($user_roles))<div><x-label for="role">Role</x-label><x-select width="compact" name="role" id="role"><option value="">System default</option>@foreach($user_roles as $roleId => $roleName)<option value="{{ $roleId }}" @selected((string) old('role') === (string) $roleId)>{{ $roleName }}</option>@endforeach</x-select></div>@endif
