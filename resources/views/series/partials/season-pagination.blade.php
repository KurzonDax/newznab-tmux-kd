@if(!empty($pagination['per_page']) && $pagination['per_page'] > 0 && ($pagination['total_pages'] ?? 1) > 1)
    <div class="series-stat-card mt-6 flex items-center justify-between bg-(--surface-panel-alt) dark:bg-(--surface-body-dark) border border-gray-200 dark:border-gray-700 rounded-lg p-4" data-series-pagination>
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ $prevUrl }}"
               ...
               class="px-4 py-2 rounded-lg text-sm font-medium {{ $pagination['current_page'] > 1 ? 'bg-(--surface-card) dark:bg-(--surface-card-dark) border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 hover:bg-(--public-surface-hover) dark:hover:bg-(--public-surface-hover-dark)' : 'bg-(--surface-panel-alt) dark:bg-(--surface-panel-alt-dark) text-gray-400 cursor-not-allowed' }}">
                <i class="fas fa-arrow-left mr-2"></i>Previous
            </a>
            <a href="{{ $nextUrl }}"
               @if($pagination['current_page'] < $pagination['total_pages']) data-series-pagination-link @endif
               class="px-4 py-2 rounded-lg text-sm font-medium {{ $pagination['current_page'] < $pagination['total_pages'] ? 'bg-primary-600 text-white hover:bg-primary-700' : 'bg-(--surface-panel-alt) dark:bg-(--surface-panel-alt-dark) text-gray-400 cursor-not-allowed' }}">
