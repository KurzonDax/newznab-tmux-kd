            ...
            @if(!empty($show))
                ...
                <div class="details-metadata-section">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @if(!empty($showTitle))
                            <div>
                                <dt class="text-[11px] font-medium text-gray-600 dark:text-gray-400">Show Title</dt>
                                <dd class="mt-1 text-[13px] text-gray-900 dark:text-gray-100 font-semibold">{{ $showTitle }}</dd>
                            </div>
                        @endif
                        @if(!empty($showStarted))
                            <div>
                                <dt class="text-[11px] font-medium text-gray-600 dark:text-gray-400">Started</dt>
                                <dd class="mt-1 text-[13px] text-gray-900 dark:text-gray-100">{{ $showStarted }}</dd>
                            </div>
                        @endif
                        @if(!empty($showTvdb))
                            <div>
