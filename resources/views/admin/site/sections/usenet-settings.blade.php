                ...
                <div class="border-b border-gray-200 dark:border-gray-700 pb-6">
                    ...
                    <p class="text-sm text-gray-500 mb-4">
                        How hard the scheduled passes work to rescue an incomplete release before the completion sweep
                    ...
                    <div class="space-y-4">
                        <div>
                            <label for="repair_retry_after_hours" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            ...
                            <p class="mt-1 text-sm text-gray-500">How long after a repair pass falls short the final pass may run. Fresh releases are promoted at the collection timeout and repaired within hours, while their articles may still be propagating across the provider farm, so a first attempt can fail where a recheck days later succeeds. Every release gets two passes at most.</p>
                        </div>

                        <div>
                            <label for="repair_floor_completion" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            ...
                            <p class="mt-1 text-sm text-gray-500">Releases measured below this percentage skip network repair entirely and go straight to a final outcome. A release holding under a tenth of its articles is not a header-scan miss, and confirming that costs article probes.</p>
                        </div>

                        <div>
                            <label for="repair_stat_sample_per_file" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            ...
                            <p class="mt-1 text-sm text-gray-500">Synthesized message-IDs spot-checked per file before its segments are written into the NZB. A file is accepted only when every sampled ID exists. One sample cannot say whether the message-ID template was guessed correctly, so two is the sensible minimum.</p>
                        </div>

                        <div>
                            <label for="repair_max_stat_probes" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            ...
                            <p class="mt-1 text-sm text-gray-500">Hard ceiling on article existence probes for one release, however many files it has. When the budget runs out mid-way the remaining files are left for the next pass rather than accepted on a thinner sample.</p>
                        </div>

                        <div>
                            <label for="repair_limit" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            ...
                            <p class="mt-1 text-sm text-gray-500">Releases one <code>releases:repair-completion</code> invocation works on. Repaired releases flow straight back into additional processing, so a large batch here starves fresh releases of post-processing capacity.</p>
                        </div>

                        <div>
                            <label for="rescan_limit" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            ...
                            <p class="mt-1 text-sm text-gray-500">Releases one <code>releases:rescan-missing-files</code> invocation works on. The header re-scan recovers files the scan missed entirely, and it competes with live header scanning for the primary provider’s connections.</p>
                        </div>

                        <div>
                            <label for="rescan_window_minutes" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            ...
                            <p class="mt-1 text-sm text-gray-500">How far either side of the release’s known article range the re-scan looks, in posting time. Widening it finds files posted further from the rest of the collection, at a proportional cost in overview lines fetched.</p>
                        </div>

                        <div>
                            <label for="rescan_max_articles_per_release" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            ...
                            <p class="mt-1 text-sm text-gray-500">A release whose estimated article range is wider than this is stamped as skipped without fetching anything. Low-traffic groups give tight ranges; a busy group over a wide window can span millions of articles for one release.</p>
                        </div>

                        <div>
                            <label for="rescan_max_articles_per_run" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            ...
                            <p class="mt-1 text-sm text-gray-500">The invocation stops fetching once this many overview lines have been read, whatever is left in the batch. The unfinished releases keep their state and are picked up next run.</p>
                        </div>
                    </div>
                </div>
