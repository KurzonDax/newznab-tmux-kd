...
<div x-data="toastContainer"
     @if($publicToasts ?? false) data-public-toasts @endif
     @class([
         'public-toasts' => $publicToasts ?? false,
         'fixed top-4 right-4 z-9999 flex flex-col gap-2 pointer-events-none' => ! ($publicToasts ?? false),
     ])
     aria-live="polite"
     aria-atomic="true">
    <template x-for="toast in items()" :key="toast.id">
        <div @class([
                 'card public-toast' => $publicToasts ?? false,
                 'pointer-events-auto max-w-sm w-full bg-white dark:bg-gray-800 shadow-lg rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden transform transition-all duration-300' => ! ($publicToasts ?? false),
             ])
