...
### G30 — repair/rescan writes never reach the search index, and the only general reconciliation is dead code (high, stale search)

**Reachable state and path.** Every state write in both recovery engines uses `Release::query()->...->update()` (`ReleaseRepairService.php:197-201,254-263`; `MissingFileRescanService.php:413-422`), which bypasses model events, so `ReleaseObserver` — the sole search-sync path — never fires. A repair-requeued release carries stale `haspreview`/`passwordstatus` (both indexed in `releases_rt` and filtered on by the driver: `ManticoreSearchDriver.php:2566-2568,2864-2872,2908-2914`) and stale `completion` indefinitely.

**Absence.** `nntmux:search-repair` (scheduled every minute, `routes/console.php:70`) retries only failures the Manticore driver itself recorded into `search_index_failures` (`ManticoreSearchDriver.php:435-455`) — it cannot see updates that never attempted a sync. `SearchIndexOutboxDispatcher` explicitly presumes an outbox "written outside PHP (database triggers)", but no trigger, outbox table, or consumer exists anywhere in the tree. Consequences of G29, or a crash between DB commit and the `afterCommit` sync, are never detected short of a manual `nntmux:populate`.
...
**Exclusion.** A release AP never reaches — size-excluded above the maximum, or stranded per G5/G23/G24 — is excluded from the standard sweep indefinitely, the same shape as G1b's `nfostatus` gate. The windowed per-source methods do not check `passwordstatus`, so only the sweep (the safety net that matters after the six-hour windows expire) is affected.

### G32 — SRRDB is missing from the standard sweep (medium, stranded names)

**Exclusion.** `standardCandidateBatch`'s OR list (`NameFixingQueryService.php:134-142`) has no `proc_srrdb = 0` term, and tmux runs SRRDB only as level 21 inside the six-hour window (`FixReleaseNames.php:121-122`; `TmuxTaskRunner.php:438-447`). The comment at `TmuxTaskRunner.php:430-432` — that anything a stalled method let age out is still picked up by the sweep — is false for SRRDB: releases older than six hours that never got an SRRDB pass (e.g. during an SRRDB API outage) are never retried automatically.
...
| G15 — Decide forced-root precedence for audio-only MediaInfo evidence | Medium | `needs-triage` | #140/#141 makes group force absolute (and locks it in by test), while ADR 0006 declares audio-only evidence the sole cross-root exception. Choose and document the precedence — amend ADR 0006 or reverse the guard — then align pipeline, direct audio rename, and refinement. **Acceptance once chosen:** tests cover forced XXX/Movie/TV groups with audio-only evidence plus video controls, and prove every categorization entry point yields the same specified root. |
...
| G31 — Decouple the standard name sweep from the password pending sentinel | Medium | `ready-for-agent` | `passwordstatus>=0` silently excludes inspection-pending rows from the sweep, the same shape as the NFO gate. Let non-password evidence proceed while AP still owns password settlement. **Acceptance:** query tests prove a `-1` row with unprocessed evidence is selected and password inspection state is unchanged by naming. |
| G32 — Add SRRDB to the standard sweep | Medium | `ready-for-agent` | The sweep's OR list omits `proc_srrdb`, so SRRDB work that ages past the six-hour window is never retried. Include it under the same trusted-name/config gates as level 21. **Acceptance:** query tests prove an aged eligible row is selected only when SRRDB is configured, and processed/ambiguous rows remain excluded. |
