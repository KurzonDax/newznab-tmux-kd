...
final class ProductionTablesTest extends TestCase
{
    ...
    public function test_builder_tables_satisfy_the_guard(string $table, ?array $columns): void
    {
        ...
        self::assertEqualsCanonicalizing($expected, $this->columnNames($pdo, $table));
    }
    ...
    public function test_unknown_tables_and_columns_fail_by_name(): void
    {
        ...
        foreach ([
            ...
            [static fn () => $tables->createStatement('releases', ['id', 'invented_column', 'invented_flag']), 'releases has no column invented_column, invented_flag'],
            [static fn () => $tables->createStatement('releases', []), 'releases needs at least one column'],
            [static fn () => $tables->createStatement('releases', ['id', 'name_evidence_work_pending']), 'releases.name_evidence_work_pending is generated from proc_crc32, proc_files, proc_media_movie, proc_srr, proc_srrdb, proc_uid, proc_xxx'],
            [static fn () => $tables->createStatement('pulse_values'), 'pulse_values.key_hash is generated by an expression SQLite cannot evaluate'],
        ] as [$build, $message]) {
            try {
                $build();
    ...
    public function test_output_follows_a_changed_authority_without_editing_the_builder(): void
    {
        ...
        self::assertSame(['id', 'name'], $this->columnNames($pdo, 'widgets'));
        self::assertSame([], $this->uniqueIndexes($pdo, 'widgets'));

        $pdo = new PDO('sqlite::memory:');
        $pdo->exec($after->createStatement('widgets'));
        self::assertSame(['id', 'name', 'owner_id'], $this->columnNames($pdo, 'widgets'));
    ...
    /** @return list<string> */
    private function columnNames(PDO $pdo, string $table): array
    {
        return array_column($pdo->query("PRAGMA table_xinfo($table)")->fetchAll(PDO::FETCH_ASSOC), 'name');
    }
    ...
    /** @return array<string, array<string, mixed>> As read by {@see SchemaAuthority::tables()}. */
