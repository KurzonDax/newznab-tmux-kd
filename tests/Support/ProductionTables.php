...
/**
 * Creates SQLite fixture tables from the schema authority, so they follow the production dump:
 * real column names, every production primary/unique key whose columns are present, the
 * auto-increment identity, SQLite type affinity, literal defaults and generated columns.
 * Every column is nullable; foreign keys, checks and non-unique indexes are not reproduced.
 */
final class ProductionTables
{
    private const array TYPES = [
        'INTEGER' => ['tinyint', 'smallint', 'mediumint', 'int', 'integer', 'bigint', 'bit', 'year'],
        'TEXT' => ['char', 'varchar', 'tinytext', 'text', 'mediumtext', 'longtext', 'enum', 'set', 'json', 'uuid'],
        'REAL' => ['float', 'double'],
        'NUMERIC' => ['decimal'],
    ...
    private static ?self $authority = null;
    ...
    {
        return self::$authority ??= new self(new SchemaAuthority(dirname(__DIR__, 2).'/database/schema/mariadb-schema.sql'));
    }
